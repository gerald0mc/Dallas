...
public class RotationManager {
    ...
    public void rotateToEntity(Entity entity) {
        final float[] angle = MathUtils.calcAngle(Minecraft.getMinecraft().player.getPositionEyes(Minecraft.getMinecraft().getRenderPartialTicks()), entity.getPositionVector());
        shouldRotate = true;
        desiredYaw = angle[0];
    ...
    public void rotateToPosition(BlockPos position) {
        final float[] angle = MathUtils.calcAngle(Minecraft.getMinecraft().player.getPositionEyes(Minecraft.getMinecraft().getRenderPartialTicks()), new Vec3d(position.getX() + 0.5f, position.getY() - 0.5f, position.getZ()));
        shouldRotate = true;
        desiredYaw = angle[0];
        desiredPitch = angle[1];
    }
